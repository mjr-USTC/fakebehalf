...
def parse_args():
    ...
    parser.add_argument(
		'-to', '--victim',  default = config["victim_address"], help = "Set the receiver manually in server mode")
...
def main():
	...
	if args.victim:
		config['victim_address'] = args.victim.encode("utf-8")

	if check_configs() == -1:
		return -1
	
	# used for recording the log files
	case_id = args.caseid
	destination = config['victim_address'].split(b'@')[1].decode()
	output_dir = f"log/{destination}"
	if not os.path.exists(output_dir):
		os.makedirs(output_dir)
	log_name = f'{case_id}.log'
